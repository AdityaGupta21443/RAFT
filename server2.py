...
# nodes["node4"]=bruh_pb2_grpc.NodeStub(grpc.insecure_channel('localhost:50048'))
# nodes["node5"]=bruh_pb2_grpc.NodeStub(grpc.insecure_channel('localhost:50049'))
...
def write_to_meta():
    ...
    # commitindex=log[len(log)-1][2]
    # lastApplied=log[len(log)-1][3]
...
def heartbeat():
    global hb,acks,commitindex,lastApplied,node_num
    global term
    ...
    def heart(node_no):
        ...
        while(n<l):
            if n>len(log)-1:
                ...
                request.entries+=f"{log[n][0]},{log[n][1]},{log[n][2]},{log[n][3]} "
                n+=1
    ...
    with lock2:
        if not hb:
            hb=True
            lock1.release()
    if acks>=1:# how to get entry for which 
        acks=0
...
def leader_job():
    global term,acks,t4,lastApplied,leader
    leader=node_num
...
hb=True
def reqVote():
    global term,vote_casted,role,voted_for,node_num
    term+=1
    role="candidate"
    global votes
    votes=1
